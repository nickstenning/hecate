...
def render_to_string_with_autoescape_off(template_name,
                                         dictionary=None,
...
def render_to_email(to,
                    ...
                    opt_out=True):
    """
    Send a multipart email using the given templates. Note that to should be
    either User objects or email addresses as strings.
    ...
    if subject is None:
        subject = render_to_string_with_autoescape_off([subject_template],
                                                       context)
